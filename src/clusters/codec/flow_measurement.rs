//! Matter TLV encoders and decoders for Flow Measurement Cluster
//! Cluster IDs: 0x0404
//!
//! This file is automatically generated from FlowMeasurement.xml
...
pub fn decode_attribute_json(cluster_id: u32, attribute_id: u32, tlv_value: &crate::tlv::TlvItemValue) -> String {
    ...
    if ![0x0404].contains(&cluster_id) {
        return format!("{{\"error\": \"Invalid cluster ID. Expected [0x0404], got {}\"}}", cluster_id);
    }
...
/// Read `MeasuredValue` attribute from cluster `Flow Measurement`.
pub async fn read_measured_value(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<Option<u16>> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_FLOW_MEASUREMENT, crate::clusters::defs::CLUSTER_FLOW_MEASUREMENT_ATTR_ID_MEASUREDVALUE).await?;
    decode_measured_value(&tlv)
}

/// Read `MinMeasuredValue` attribute from cluster `Flow Measurement`.
pub async fn read_min_measured_value(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<Option<u16>> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_FLOW_MEASUREMENT, crate::clusters::defs::CLUSTER_FLOW_MEASUREMENT_ATTR_ID_MINMEASUREDVALUE).await?;
    decode_min_measured_value(&tlv)
}

/// Read `MaxMeasuredValue` attribute from cluster `Flow Measurement`.
pub async fn read_max_measured_value(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<Option<u16>> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_FLOW_MEASUREMENT, crate::clusters::defs::CLUSTER_FLOW_MEASUREMENT_ATTR_ID_MAXMEASUREDVALUE).await?;
    decode_max_measured_value(&tlv)
}

/// Read `Tolerance` attribute from cluster `Flow Measurement`.
pub async fn read_tolerance(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u16> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_FLOW_MEASUREMENT, crate::clusters::defs::CLUSTER_FLOW_MEASUREMENT_ATTR_ID_TOLERANCE).await?;
