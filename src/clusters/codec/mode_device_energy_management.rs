//! Matter TLV encoders and decoders for Device Energy Management Mode Cluster
//! Cluster IDs: 0x009F
//!
...
impl ModeTag {
    ...
    pub fn from_u16(value: u16) -> Option<Self> {
        match value {
            ...
            9 => Some(ModeTag::Day),
            16384 => Some(ModeTag::Nooptimization),
            16385 => Some(ModeTag::Deviceoptimization),
            16386 => Some(ModeTag::Localoptimization),
            16387 => Some(ModeTag::Gridoptimization),
            _ => None,
        }
    }
...
/// Read `SupportedModes` attribute from cluster `Device Energy Management Mode`.
pub async fn read_supported_modes(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DEVICE_ENERGY_MANAGEMENT_MODE, crate::clusters::defs::CLUSTER_DEVICE_ENERGY_MANAGEMENT_MODE_ATTR_ID_SUPPORTEDMODES).await?;
    decode_supported_modes(&tlv)
}

/// Read `CurrentMode` attribute from cluster `Device Energy Management Mode`.
pub async fn read_current_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DEVICE_ENERGY_MANAGEMENT_MODE, crate::clusters::defs::CLUSTER_DEVICE_ENERGY_MANAGEMENT_MODE_ATTR_ID_CURRENTMODE).await?;
    decode_current_mode(&tlv)
}

/// Read `StartUpMode` attribute from cluster `Device Energy Management Mode`.
pub async fn read_start_up_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DEVICE_ENERGY_MANAGEMENT_MODE, crate::clusters::defs::CLUSTER_DEVICE_ENERGY_MANAGEMENT_MODE_ATTR_ID_STARTUPMODE).await?;
    decode_start_up_mode(&tlv)
}

/// Read `OnMode` attribute from cluster `Device Energy Management Mode`.
pub async fn read_on_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DEVICE_ENERGY_MANAGEMENT_MODE, crate::clusters::defs::CLUSTER_DEVICE_ENERGY_MANAGEMENT_MODE_ATTR_ID_ONMODE).await?;
    decode_on_mode(&tlv)
}
