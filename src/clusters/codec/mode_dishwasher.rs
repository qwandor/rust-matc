//! Matter TLV encoders and decoders for Dishwasher Mode Cluster
//! Cluster IDs: 0x0059
//!
//! This file is automatically generated from Mode_Dishwasher.xml
...
impl ModeTag {
    ...
    pub fn from_u16(value: u16) -> Option<Self> {
        match value {
            ...
            16385 => Some(ModeTag::Heavy),
            16386 => Some(ModeTag::Light),
            _ => None,
        }
    }
...
pub fn decode_attribute_json(cluster_id: u32, attribute_id: u32, tlv_value: &crate::tlv::TlvItemValue) -> String {
    ...
    if ![0x0059].contains(&cluster_id) {
        return format!("{{\"error\": \"Invalid cluster ID. Expected [0x0059], got {}\"}}", cluster_id);
    }
...
/// Read `SupportedModes` attribute from cluster `Dishwasher Mode`.
pub async fn read_supported_modes(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DISHWASHER_MODE, crate::clusters::defs::CLUSTER_DISHWASHER_MODE_ATTR_ID_SUPPORTEDMODES).await?;
    decode_supported_modes(&tlv)
}

/// Read `CurrentMode` attribute from cluster `Dishwasher Mode`.
pub async fn read_current_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DISHWASHER_MODE, crate::clusters::defs::CLUSTER_DISHWASHER_MODE_ATTR_ID_CURRENTMODE).await?;
    decode_current_mode(&tlv)
}

/// Read `StartUpMode` attribute from cluster `Dishwasher Mode`.
pub async fn read_start_up_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DISHWASHER_MODE, crate::clusters::defs::CLUSTER_DISHWASHER_MODE_ATTR_ID_STARTUPMODE).await?;
    decode_start_up_mode(&tlv)
}

/// Read `OnMode` attribute from cluster `Dishwasher Mode`.
pub async fn read_on_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_DISHWASHER_MODE, crate::clusters::defs::CLUSTER_DISHWASHER_MODE_ATTR_ID_ONMODE).await?;
    decode_on_mode(&tlv)
}
