//! Matter TLV encoders and decoders for Energy EVSE Mode Cluster
//! Cluster IDs: 0x009D
//!
//! This file is automatically generated from Mode_EVSE.xml
...
impl ModeTag {
    ...
    pub fn from_u16(value: u16) -> Option<Self> {
        match value {
            ...
            9 => Some(ModeTag::Day),
            16384 => Some(ModeTag::Manual),
            16385 => Some(ModeTag::Timeofuse),
            16386 => Some(ModeTag::Solarcharging),
            16387 => Some(ModeTag::V2x),
            _ => None,
        }
    }
...
pub fn decode_attribute_json(cluster_id: u32, attribute_id: u32, tlv_value: &crate::tlv::TlvItemValue) -> String {
    ...
    if ![0x009D].contains(&cluster_id) {
        return format!("{{\"error\": \"Invalid cluster ID. Expected [0x009D], got {}\"}}", cluster_id);
    }
...
/// Read `SupportedModes` attribute from cluster `Energy EVSE Mode`.
pub async fn read_supported_modes(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_ENERGY_EVSE_MODE, crate::clusters::defs::CLUSTER_ENERGY_EVSE_MODE_ATTR_ID_SUPPORTEDMODES).await?;
    decode_supported_modes(&tlv)
}

/// Read `CurrentMode` attribute from cluster `Energy EVSE Mode`.
pub async fn read_current_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_ENERGY_EVSE_MODE, crate::clusters::defs::CLUSTER_ENERGY_EVSE_MODE_ATTR_ID_CURRENTMODE).await?;
    decode_current_mode(&tlv)
}

/// Read `StartUpMode` attribute from cluster `Energy EVSE Mode`.
pub async fn read_start_up_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_ENERGY_EVSE_MODE, crate::clusters::defs::CLUSTER_ENERGY_EVSE_MODE_ATTR_ID_STARTUPMODE).await?;
    decode_start_up_mode(&tlv)
}

/// Read `OnMode` attribute from cluster `Energy EVSE Mode`.
pub async fn read_on_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_ENERGY_EVSE_MODE, crate::clusters::defs::CLUSTER_ENERGY_EVSE_MODE_ATTR_ID_ONMODE).await?;
    decode_on_mode(&tlv)
}
