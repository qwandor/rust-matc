//! Matter TLV encoders and decoders for RVC Run Mode Cluster
//! Cluster IDs: 0x0054
//!
...
impl ModeTag {
    ...
    pub fn from_u16(value: u16) -> Option<Self> {
        match value {
            ...
            9 => Some(ModeTag::Day),
            16384 => Some(ModeTag::Idle),
            16385 => Some(ModeTag::Cleaning),
            16386 => Some(ModeTag::Mapping),
            _ => None,
        }
    }
...
impl StatusCode {
    ...
    pub fn from_u8(value: u8) -> Option<Self> {
        match value {
            65 => Some(StatusCode::Stuck),
            66 => Some(StatusCode::Dustbinmissing),
            67 => Some(StatusCode::Dustbinfull),
            68 => Some(StatusCode::Watertankempty),
            69 => Some(StatusCode::Watertankmissing),
            70 => Some(StatusCode::Watertanklidopen),
            71 => Some(StatusCode::Mopcleaningpadmissing),
            72 => Some(StatusCode::Batterylow),
            _ => None,
        }
    }
...
pub fn decode_attribute_json(cluster_id: u32, attribute_id: u32, tlv_value: &crate::tlv::TlvItemValue) -> String {
    ...
    if ![0x0054].contains(&cluster_id) {
        return format!("{{\"error\": \"Invalid cluster ID. Expected [0x0054], got {}\"}}", cluster_id);
    }
...
/// Read `SupportedModes` attribute from cluster `RVC Run Mode`.
pub async fn read_supported_modes(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_RVC_RUN_MODE, crate::clusters::defs::CLUSTER_RVC_RUN_MODE_ATTR_ID_SUPPORTEDMODES).await?;
    decode_supported_modes(&tlv)
}

/// Read `CurrentMode` attribute from cluster `RVC Run Mode`.
pub async fn read_current_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_RVC_RUN_MODE, crate::clusters::defs::CLUSTER_RVC_RUN_MODE_ATTR_ID_CURRENTMODE).await?;
    decode_current_mode(&tlv)
}

/// Read `StartUpMode` attribute from cluster `RVC Run Mode`.
pub async fn read_start_up_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_RVC_RUN_MODE, crate::clusters::defs::CLUSTER_RVC_RUN_MODE_ATTR_ID_STARTUPMODE).await?;
    decode_start_up_mode(&tlv)
}

/// Read `OnMode` attribute from cluster `RVC Run Mode`.
pub async fn read_on_mode(conn: &crate::controller::Connection, endpoint: u16) -> anyhow::Result<u8> {
    let tlv = conn.read_request2(endpoint, crate::clusters::defs::CLUSTER_ID_RVC_RUN_MODE, crate::clusters::defs::CLUSTER_RVC_RUN_MODE_ATTR_ID_ONMODE).await?;
    decode_on_mode(&tlv)
}
