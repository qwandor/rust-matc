...
from .xml_parser import ClusterParser, get_cluster_ids
...
def generate_json_dispatcher_function(cluster_ids: List[str], attributes: List[AttributeField], structs: Dict[str, MatterStruct]) -> str:
    ...
    cluster_ids_list = ', '.join(cluster_ids)
...
pub fn decode_attribute_json(cluster_id: u32, attribute_id: u32, tlv_value: &crate::tlv::TlvItemValue) -> String {{
    ...
    if ![{cluster_ids_list}].contains(&cluster_id) {{
        return format!("{{{{\\\"error\\\": \\\"Invalid cluster ID. Expected [{cluster_ids_list}], got {{}}\\\"}}}}", cluster_id);
    }}
...
def generate_attribute_list_function(cluster_ids: List[str], attributes: List[AttributeField]) -> str:
...
def generate_event_json_dispatcher_function(cluster_ids: List[str], events) -> str:
...
def generate_event_list_function(cluster_ids: List[str], events) -> str:
...
def generate_rust_code(xml_file: str, typedefs: Optional[Dict[str, str]] = None) -> str:
    ...
    cluster_ids_str = ', '.join(parser.cluster_ids)
    code = f'''//! Matter TLV encoders and decoders for {parser.cluster_name}
//! Cluster IDs: {cluster_ids_str}
//!
...
'''
    ...
    if attributes:
        ...
        code += generate_json_dispatcher_function(parser.cluster_ids, attributes, structs)

        # Generate attribute list function
        code += generate_attribute_list_function(parser.cluster_ids, attributes)
    ...
    if events:
        ...
        code += generate_event_json_dispatcher_function(parser.cluster_ids, events)
        code += generate_event_list_function(parser.cluster_ids, events)
...
def process_xml_files(xml_dir: str, output_dir: str) -> None:
    ...
    for xml_file in sorted(xml_files):
        ...
        try:
            ...
            cluster_ids = get_cluster_ids(root)
